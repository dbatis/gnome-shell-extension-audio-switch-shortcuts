...
export default class AudioSwitchShortCutsExtension extends Extension {
		...
		new MixerSource()
			...
			.then((mixer) => {
				...
				this.mixer
					...
					.forEach((device) => {
						this.deviceSettings?.addOrEnableDevice(
							device.name,
							device.id,
							DeviceType.OUTPUT,
						);
				...
				this.mixerSubscription = this.mixer.subscribeToDeviceChanges(
					(event) => {
						try {
							const device = this.mixer?.getAudioDevicesFromIds(
								[event.deviceId],
								event.type,
							)?.[0];
							...
							if (event.action === Action.ADDED) {
								...
								this.deviceSettings?.disableDevice(
									device.name,
									event.type,
								);
							}
	...
	async sendNotification(device: StoredDevice) {
		if (
			this.extensionSettings!.get_boolean(Constants.KEY_NOTIFICATIONS) &&
			!this.extensionSettings!.get_boolean(Constants.KEY_SHOW_VOLUME_OSD)
		) {
			if (
				this.notificationSource &&
				this.notificationSource!.notifications.length > 0
			) {
				this.notificationSource!.notifications.forEach((n) =>
			...
			const message =
				alternateName !== undefined ? alternateName : device.name;
	...
	async playAudioAlert() {
		if (
			this.extensionSettings!.get_boolean(Constants.KEY_PLAY_AUDIO_ALERT)
		) {
	...
	private createNotificationSource() {
		if (this.notificationSource === undefined) {
			...
			this.notificationDestroySignal = this.notificationSource.connect(
				"destroy",
				(_) => {
	...
	disable() {
		...
		if (this.notificationSource) {
			if (this.notificationDestroySignal) {
				this.notificationSource.disconnect(
					this.notificationDestroySignal,
				);
